...
class FoldSplitter:
    ...
    def user_based_split(self, spark, history, bag_of_words, paperId_col="paper_id" , userId_col = "user_id", tf_map_col = "term_occurrence", fold_num = 5):
        ...
        # # store paper corpus
        # papers_corpus.papers.write.csv(os.path.join(self.output_dir, '{}_folds'.format(self.split_method), Fold.PAPER_CORPUS_DF_CSV_FILENAME))
        ...
        # # save lda paper profiles
        # # parquet used because we cannot store vectors (lda vectors) in csv format
        # ldaModel.paper_profiles.write.parquet(os.path.join(self.output_dir, '{}_folds'.format(self.split_method), Fold.LDA_DF_FILENAME))
        ...
        for fold_index in range(fold_num):
            ...
            fold = Fold(training_data_frame, test_data_frame, os.path.join(self.output_dir, '{}_folds'.format(self.split_method)), self.split_method)
...
class FoldsUtils:
    ...
    def compute_fold_statistics(fold, userId_col="user_id", paperId_col="paper_id"):
        ...
        formatted_line = line.format(fold.index, fold_time, total_users_count, tr_users_count, test_users_count, diff_users_count, new_users_count, \
                                     ...
                                     "/" + str(tr_avg_ratings_per_user) + "/" + str(tr_std_ratings_per_user) \
                                     , str(ts_min_ratings_per_user) + "/" + str(ts_max_ratings_per_user) + "/" +
                                     str(ts_avg_ratings_per_user) + "/" + str(ts_std_ratings_per_user), \
                                     str(tr_min_ratings_per_item) + "/" + str(tr_max_ratings_per_item) + "/" +
                                     str(tr_avg_ratings_per_item) + "/" + str(tr_std_ratings_per_item), \
                                     str(ts_min_ratings_per_item) + "/" + str(ts_max_ratings_per_item) + "/" +
                                     str(ts_avg_ratings_per_item) + "/" + str(ts_std_ratings_per_item))
...
class FoldValidator():
    ...
    def __init__(self, peer_papers_count = 10, pairs_generation = "edp", pairs_features_generation_method= "sub",  paperId_col = "paper_id",
                 userId_col = "user_id", model_training = "gm", output_dir = 'results', split_method = 'time-aware', min_peer_similarity = None):
        """
    ...
    def create_folds(self, spark, history, bag_of_words, tf_map_col="term_occurrence", timestamp_col="timestamp", fold_period_in_months=6):
        ...
        splitter = FoldSplitter(self.split_method, self.output_dir)
    ...
    def load_fold(self, spark, fold_index, distributed=True):
        ...
        if self.split_method == 'user-based' or self.split_method == 'item-based':
            ...
            training_data_frame = spark.read.csv(os.path.join(self.output_dir, '{}_folds'.format(self.split_method), Fold.get_training_data_frame_path(fold_index, distributed)), header=False, schema=fold_schema)
        ...
        if self.split_method == 'time-aware':
            ...
            # load test data frame
            test_data_frame = spark.read.csv(os.path.join(self.output_dir, '{}_folds'.format(self.split_method), Fold.get_test_data_frame_path(fold_index, distributed)), header=False,
            ...
            training_data_frame = spark.read.csv(os.path.join(self.output_dir, '{}_folds'.format(self.split_method), Fold.get_training_data_frame_path(fold_index, distributed)), header=False,
            ...
            # load papers corpus
            papers = spark.read.csv(os.path.join(self.output_dir, '{}_folds'.format(self.split_method), Fold.get_papers_corpus_frame_path(fold_index, distributed)),
    ...
    def evaluate_folds(self, spark, folds=range(1,6)):
        ...
        fold_evaluator = FoldEvaluator(output_dir=self.output_dir, k_mrr=[5, 10], k_ndcg=[5, 10],
                                       ...
                                       min_sim=self.min_peer_similarity,
                                       split_method = self.split_method,
                                       pairs_features_generation_method = self.pairs_features_generation_method)
        ...
        for i in folds:
            ...
            file.write("Peer features modeling: " + str(self.pairs_features_generation_method) + "\n")
            file.close()
...
class FoldEvaluator:
    ...
    def __init__(self, output_dir,  k_mrr = [5, 10], k_recall = [x for x in range(5, 200, 20)], k_ndcg = [5, 10] , model_training = "gm", peers_count = 1, pairs_generation = "edp", min_sim=None, split_method = 'time-aware', pairs_features_generation_method = 'sub'):
        self.k_mrr = k_mrr
        self.k_ndcg = k_ndcg
        ...
        self.file_name = os.path.join(self.output_dir, "{}_{}_{}-minsim_{}-peers_{}_{}_{}".format(self.model_training , split_method, min_sim, self.peers_count, self.pairs_generation, pairs_features_generation_method, self.RESULTS_CSV_FILENAME))
